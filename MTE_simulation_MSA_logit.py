...
def phi(z: np.ndarray, beta: float = 0.5) -> float:
    """Logit choice function"""
    z = np.array(z)
    return -1 / beta * np.log(np.sum(np.exp(-beta * z)))

def d_phi(z: np.ndarray, beta: float = 0.5) -> np.ndarray:
    """Derivative of logit function (choice probabilities)"""
    z = np.array(z)
    exp = np.exp(-beta * z)
...
def run_sioux_falls_convergence_log(beta):
    ...
    plt.tight_layout()
    plt.savefig(f"mte_convergence_MSA_logit_{beta}.png", dpi=300)
    plt.show()
...
if __name__ == "__main__":
    ...
    w = run_sioux_falls_convergence_log(beta)
    end_time = time.time()
